...
export function StockTransferForm() {
  ...
  const canProceedToItems = fromLocation && toLocation && fromLocation !== toLocation
  const hasStockIssues = transferItems.some((item) => item.quantity > item.availableStock)
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            {steps.map((step, index) => {
              ...
              const isCompleted =
                (step.id === "details" && (currentStep === "items" || currentStep === "review")) ||
              ...
              return (
                <div key={step.id} className="flex flex-1 items-center">
                  ...
                  {index < steps.length - 1 && (
                    <div className={`mx-4 h-px flex-1 ${isCompleted ? "bg-primary" : "bg-border"}`} />
                  )}
                </div>
              )
            })}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-end">
              <Button onClick={() => setCurrentStep("items")} disabled={!canProceedToItems}>
                Next: Select Items
              </Button>
            </div>
          </CardContent>
                    ...
                    {transferItems.map((item) => {
                      ...
                      return (
                        <TableRow key={item.id} className={exceedsStock ? "bg-destructive/10" : ""}>
                          ...
                          <TableCell className="text-sm text-muted-foreground">{item.availableStock}</TableCell>
                          <TableCell>
                            <Button type="button" variant="ghost" size="sm" onClick={() => removeTransferItem(item.id)}>
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Review transfer details before submitting</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    <div className="text-sm font-medium text-muted-foreground">From Location</div>
                    <div className="mt-1 capitalize">{fromLocation.replace("-", " ")}</div>
                  </div>
                  <div>
                    <div className="text-sm font-medium text-muted-foreground">To Location</div>
                    <div className="mt-1 capitalize">{toLocation.replace("-", " ")}</div>
                  </div>
                </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>{transferItems.length} item(s) to transfer</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="rounded-md border">
                  <Table>
