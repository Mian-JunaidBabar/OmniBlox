...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Initialize auth state - validate session cookie with backend
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        // Call /auth/me to validate the session cookie
        const validatedUser = await api.getProfile();
        ...
        if (statusCode === 401 || statusCode === 403) {
          ...
          // For non-auth errors (e.g., network/5xx), keep state as logged out
  ...
  const logout = async (): Promise<void> => {
    ...
    try {
      // Call backend to destroy session and clear cookie
      await api.logout();
    } catch (error) {
