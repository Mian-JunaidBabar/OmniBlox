...
export function useProductApi() {
  ...
  const getProducts = async (filters: ProductFilters = {}): Promise<ProductListResponse> => {
    ...
    return get(`/products${query ? `?${query}` : ''}`) as Promise<ProductListResponse>
  }
  ...
  const updateProduct = async (id: string, data: UpdateProductData): Promise<Product> => {
  ...
  const updateStock = async (
    ...
    return put(`/products/${id}/stock`, { quantity, operation }) as Promise<Product>
  }
