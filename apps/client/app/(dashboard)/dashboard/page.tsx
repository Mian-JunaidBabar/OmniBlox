...
import { Package, FileText, Warehouse, TrendingUp, BarChart3, PieChart, Users, ShoppingCart, Star } from "lucide-react"
import {
  BarChart,
  Bar,
  LineChart,
...
export default function DashboardPage() {
  ...
  // Best sellers data
  const bestSellers = [
    { rank: 1, product: "Laptop Pro 15\"", sku: "LP15-001", sales: 245, revenue: "$294,000", growth: "+15%" },
    { rank: 2, product: "Wireless Mouse", sku: "WM-002", sales: 892, revenue: "$44,600", growth: "+28%" },
    { rank: 3, product: "USB-C Cable", sku: "UC-003", sales: 1205, revenue: "$24,100", growth: "+42%" },
    { rank: 4, product: "Monitor 27\"", sku: "M27-004", sales: 156, revenue: "$62,400", growth: "+8%" },
    { rank: 5, product: "Keyboard Mechanical", sku: "KM-005", sales: 334, revenue: "$33,400", growth: "+22%" },
  ]
                      ...
                      {topCustomers.map((customer, index) => (
                        <div key={index} className="flex items-center justify-between p-4 rounded-lg bg-gray-50 border border-gray-200">
                          ...
                          <div className="text-right">
                            <p className="font-bold text-gray-900">{customer.purchases}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-4">
                      {topSuppliers.map((supplier, index) => (
                        <div key={index} className="flex items-center justify-between p-4 rounded-lg bg-gray-50 border border-gray-200">
                          ...
                          <div className="text-right">
                            <p className="font-bold text-gray-900">{supplier.supplies}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <TabsContent value="products" className="mt-6">
                    <div className="space-y-4">
                      {bestSellers.map((product) => (
                        <div key={product.rank} className="flex items-center justify-between p-4 rounded-lg bg-gray-50 border border-gray-200">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-gray-800 text-white flex items-center justify-center font-bold">
                              {product.rank}
                            </div>
                            <div>
                              ...
                              <p className="text-sm text-gray-600">{product.sku} • {product.sales} sold</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="font-bold text-gray-900">{product.revenue}</p>
                            <p className="text-xs text-gray-600">{product.growth}</p>
                          </div>
                        </div>
                      ))}
                    </div>
